...
public class PerformanceManagerService extends SystemService {
    ...
    public void onStart() {
        if (mContext.getPackageManager().hasSystemFeature(
                ContextConstants.Features.PERFORMANCE)) {
            publishBinderService(ContextConstants.PERFORMANCE_SERVICE, mBinder);
        } else {
            Log.wtf(TAG, "Performance service started by system server but feature xml not" +
    ...
    private boolean hasAppProfiles() {
        return mNumProfiles > 0 && mPatterns != null &&
               (EVSettings.Secure.getInt(mContext.getContentResolver(),
                       EVSettings.Secure.APP_PERFORMANCE_PROFILES_ENABLED, 1) == 1);
    }
    ...
    private int getUserProfile() {
        return EVSettings.Secure.getInt(mContext.getContentResolver(),
                EVSettings.Secure.PERFORMANCE_PROFILE,
    ...
    private synchronized boolean setPowerProfileInternal(int profile, boolean fromUser) {
         ...
         */
        if (fromUser) {
            EVSettings.Secure.putInt(mContext.getContentResolver(),
                    EVSettings.Secure.PERFORMANCE_PROFILE, profile);
        }
