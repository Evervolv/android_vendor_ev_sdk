...
public class SettingsProvider extends ContentProvider {
    ...
    private static final int SETTINGS_VERSION = Build.EVERVOLV_VERSION_CODES.ACTINIUM;

    private static final Bundle NULL_SETTING = Bundle.forPair("value", null);

    // Each defined user has their own settings
    protected final SparseArray<DatabaseHelper> mDbHelpers = new SparseArray<DatabaseHelper>();
    ...
    static {
        sUriMatcher.addURI(EVSettings.AUTHORITY, DatabaseHelper.TableNames.TABLE_SYSTEM,
                SYSTEM);
        sUriMatcher.addURI(EVSettings.AUTHORITY, DatabaseHelper.TableNames.TABLE_SECURE,
                SECURE);
        sUriMatcher.addURI(EVSettings.AUTHORITY, DatabaseHelper.TableNames.TABLE_GLOBAL,
                GLOBAL);
        sUriMatcher.addURI(EVSettings.AUTHORITY, DatabaseHelper.TableNames.TABLE_SYSTEM +
    ...
    public boolean onCreate() {
        if (LOCAL_LOGV) Log.d(TAG, "Creating SettingsProvider");
        ...
        mUriBuilder.authority(EVSettings.AUTHORITY);
    ...
    public Bundle call(String method, String request, Bundle args) {
        ...
        if (args != null) {
            int reqUser = args.getInt(EVSettings.CALL_METHOD_USER_KEY, callingUserId);
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        // NOTE: update() is never called by the front-end EVSettings API, and updates that
    ...
    private void establishDbTracking(int userId) {
        ...
        synchronized (this) {
            ...
            if (LOCAL_LOGV) {
                Log.i(TAG, "Checking evervolv settings db helper for user " + userId);
            }
            if (dbHelper == null) {
                if (LOCAL_LOGV) {
                    Log.i(TAG, "Installing new evervolv settings db helper for user " + userId);
                }
    ...
    private void checkWritePermissions(String tableName) {
        if ((DatabaseHelper.TableNames.TABLE_SECURE.equals(tableName) ||
                DatabaseHelper.TableNames.TABLE_GLOBAL.equals(tableName)) &&
                getContext().checkCallingOrSelfPermission(
                        evervolv.platform.Manifest.permission.WRITE_SECURE_SETTINGS) !=
                        PackageManager.PERMISSION_GRANTED) {
            throw new SecurityException(
                    String.format("Permission denial: writing to evervolv secure settings requires %1$s",
                            evervolv.platform.Manifest.permission.WRITE_SECURE_SETTINGS));
        }
    }
    ...
    private String getTableNameFromUriMatchCode(int code) {
        switch (code) {
            case SYSTEM:
            case SYSTEM_ITEM_NAME:
                return DatabaseHelper.TableNames.TABLE_SYSTEM;
            case SECURE:
            case SECURE_ITEM_NAME:
                return DatabaseHelper.TableNames.TABLE_SECURE;
            case GLOBAL:
            case GLOBAL_ITEM_NAME:
                return DatabaseHelper.TableNames.TABLE_GLOBAL;
            default:
                throw new IllegalArgumentException("Invalid uri match code: " + code);
        }
    }
    ...
    private int getUserIdForTable(String tableName, int userId) {
        return DatabaseHelper.TableNames.TABLE_GLOBAL.equals(tableName) ?
                UserHandle.USER_OWNER : userId;
    }

    /**
     * Modify setting version for an updated table before notifying of change. The
     * {@link EVSettings} class uses these to provide client-side caches.
     * @param uri to send notifications for
     * @param userId
     */
    private void notifyChange(Uri uri, String tableName, int userId) {
        String property = null;
        final boolean isGlobal = tableName.equals(DatabaseHelper.TableNames.TABLE_GLOBAL);
        if (tableName.equals(DatabaseHelper.TableNames.TABLE_SYSTEM)) {
            property = EVSettings.System.SYS_PROP_SETTING_VERSION;
        } else if (tableName.equals(DatabaseHelper.TableNames.TABLE_SECURE)) {
            property = EVSettings.Secure.SYS_PROP_SETTING_VERSION;
        } else if (isGlobal) {
            property = EVSettings.Global.SYS_PROP_SETTING_VERSION;
        }
    ...
    private void validateSystemSettingNameValue(String name, String value) {
        EVSettings.Validator validator = EVSettings.System.VALIDATORS.get(name);
        if (validator == null) {
    ...
    private void validateSecureSettingValue(String name, String value) {
        EVSettings.Validator validator = EVSettings.Secure.VALIDATORS.get(name);
